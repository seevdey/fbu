...
--SiparisGetirici 'Speedy Express', 10, 100
 ...
 --dýþardan kaç adet çalýþan id si girildiði bilinmemektedir
...
--nested stored procedure yaptým
...
--begin
--update Products 
--set UnitsInStock += @stok where ProductID = @id --güncellemek istediðimiz kýsým
--end
--else --yoksa
--begin
...
----------------------------------------20.12 ÇALIÞMA----------------------------------------
 --ID’si verilen müþterinin toplam ne kadarlýk alýþveriþ yaptýðýný (fiyat olarak) bulan procedure
 ...
 --)
 --as
 --select SUM((1-Discount) * (od.UnitPrice*Quantity)) as [Toplam Alýþveriþ] from [Order Details] od
 --inner join Orders o on o.OrderID = od.OrderID
 --inner join Customers c  on c.CustomerID = o.CustomerID
 ...
 --Ýki tarih arasýnda bulunan sipariþler
...
--ID si verilen müþteri, en son hangi ürünü almýþ..?
...
--ID si verilen tedarikçi bana hangi ürünleri saðlýyor..?

--table value function ile tablo birleþtirme
